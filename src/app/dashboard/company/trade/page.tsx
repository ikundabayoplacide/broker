...
export default function CompanyTradePage() {
  const { user } = useAuth();
  const [companies, setCompanies] = useState<Company[]>([]);
  const [selectedCompany, setSelectedCompany] = useState<Company | null>(null);
  const [wallet, setWallet] = useState<CompanyWallet | null>(null);
  const [portfolio, setPortfolio] = useState<CompanyPortfolio[]>([]);
  ...
  // Refresh portfolio when company is selected
  useEffect(() => {
  ...
  const fetchWallet = async () => {
    try {
      ...
      if (response.ok && data.wallet) {
        setWallet({ balance: parseFloat(data.wallet.balance) });
      }
    } catch (error) {
  ...
  const fetchPortfolio = async () => {
    try {
      ...
      const response = await fetch(`/api/company/portfolio`);
      const data = await response.json();
  ...
  const fetchRecentTrades = async () => {
    try {
      ...
      const response = await fetch(`/api/company/trade/history?limit=5`);
      const data = await response.json();
  ...
  const handleTrade = async () => {
    ...
    try {
      const payload = {
        ...
        type: tradeType,
        priceType,
        ...(priceType === "LIMIT" && { requestedPrice: parseFloat(limitPrice) })
      };

      const endpoint = tradeType === "BUY" ? "/api/company/trade/buy" : "/api/company/trade/sell";
      ...
      <div className="space-y-4">
        ...
        <div className="animate-fadeInUp">
          <div className="flex justify-between items-start">
            <div>
              <h1 className="text-2xl font-bold text-gray-600">Company Trading</h1>
              <p className="text-base text-gray-400">
                Execute buy and sell orders for your company
              </p>
            </div>
            <Button
              variant="outline"
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-3 animate-slideInRight">
          <Card className="p-6 hover:shadow-lg transition-all">
            <div className="flex items-center justify-between">
              <div>
                <p className="text-base font-medium text-gray-500 mb-2">Company Wallet</p>
                {walletLoading ? (
