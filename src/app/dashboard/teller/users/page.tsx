...
export default function TellerClientsPage() {
  ...
  const fetchAssignedClients = async () => {
    try {
      setLoading(true);
      const response = await api.get("/user?role=CLIENT");
      const allUsers = Array.isArray(response.data) ? response.data : [];
      
      const processedUsers: ProcessedUser[] = allUsers.map((client: any) => ({
        id: client.id,
      ...
      <div className="space-y-6">
        ...
        <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
          <div>
            ...
            <p className="text-gray-600 mt-1">Manage clients assigned to you</p>
          </div>
              ...
              {paginatedUsers.map((user, index) => (
                ...
                >
                  ...
                  <td className="p-3">
                    <UserActions
                      user={user.raw}
                      onUserUpdated={fetchAssignedClients}
                    />
                  </td>
                </motion.tr>
              ))}
